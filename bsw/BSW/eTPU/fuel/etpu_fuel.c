  ...
  struct fuel_config_t     *p_fuel_config)
{
  ...
  eTPU->CHAN[chan_num].SCR.R = (uint32)p_fuel_instance->polarity;
  ...
  *(cpba + ((FS_ETPU_FUEL_OFFSET_PULSE_END_TIME            - 1)>>2)) = 0;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_FUEL_OFFSET_ERROR) = 0;
  *((uint8*)cpba + FS_ETPU_FUEL_OFFSET_GENERATION_DISABLE ) = p_fuel_config->generation_disable;
  ...
  struct fuel_states_t   *p_fuel_states)
{
  ...
  p_fuel_states->error                 |= *((uint8*)cpba + FS_ETPU_FUEL_OFFSET_ERROR);
  /* Clear FUEL error */
  *((uint8*)cpba + FS_ETPU_FUEL_OFFSET_ERROR) = 0;
