...
{
  const uint8  chan_num;  /**< Channel number of the SPARK channel. */
  const uint8  priority;  /**< Channel priority for the SPARK channel. */
  const uint8  polarity;  /**< SPARK polarity. It can one any of:
  ...
  const uint24_t  tdc_angle; /**< The cylinder Top Dead Center as a number
    ...
    space will start from, e.g. 0xC3FC8100. */
        uint32 *cpba_single_spark; /**< Base address of the single spark array
...
struct spark_config_t
{
  ...
  uint24_t multi_off_time;  /**< The multi-pulse OFF-time as a number of TCR1
    ticks. */
  uint8  spark_count;  /**< The count of single sparks per engine-cycle. */
...
struct single_spark_config_t
{
  ...
  uint8  multi_pulse_count;  /**< The count of multi-pulses following the
    main spark pulse. */
};
...
{
        uint8  error;     /**< This is the error status of SPARK. It may
