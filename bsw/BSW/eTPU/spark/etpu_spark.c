  ...
  struct spark_config_t     *p_spark_config)
{
  ...
  eTPU->CHAN[chan_num].SCR.R = (uint32)p_spark_instance->polarity;
  ...
  *(cpba + ((FS_ETPU_SPARK_OFFSET_P_SINGLE_SPARK_FIRST - 1)>>2)) = (uint32)cpba_single_spark - fs_etpu_data_ram_start;
  ...
  *(cpba + ((FS_ETPU_SPARK_OFFSET_END_ANGLE            - 1)>>2)) = 0;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_SPARK_COUNT        ) = spark_count;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_SPARK_COUNTER      ) = 0;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_MULTI_PULSE_COUNT  ) = 0;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_MULTI_PULSE_COUNTER) = 0;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_STATE              ) = 0;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_ERROR              ) = 0;
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_GENERATION_DISABLE ) = p_spark_config->generation_disable;
  ...
  for(i=0; i<spark_count; i++)
  {
    ...
    *(cpba_single_spark + ((FS_ETPU_SINGLE_SPARK_OFFSET_DWELL_TIME - 1)>>2)) = p_single_spark_config->dwell_time;
    /* 8-bit */
    *((uint8*)cpba_single_spark + FS_ETPU_SINGLE_SPARK_OFFSET_MULTI_PULSE_COUNT) = p_single_spark_config->multi_pulse_count;
...
*
*******************************************************************************/
uint32 fs_etpu_spark_get_states(
  struct spark_instance_t *p_spark_instance,
  struct spark_states_t   *p_spark_states)
{
  ...
  p_spark_states->error             |= *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_ERROR);
  /* Clear SPARK error */
  *((uint8*)cpba + FS_ETPU_SPARK_OFFSET_ERROR) = 0;
