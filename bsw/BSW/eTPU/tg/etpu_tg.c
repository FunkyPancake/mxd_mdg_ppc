...
{
  uint8  chan_num_crank;
  uint8  chan_num_cam;
  uint8  priority;
  uint8  cam_edge_count;
  uint8  *p_cam_edge_tooth;
  uint32 *cpba;
  uint8  *cpba8_cam_edge_tooth;
  uint8  i;
  ...
  p_cam_edge_tooth = (uint8*)p_tg_instance->p_cam_edge_tooth;
  ...
  {
    cpba8_cam_edge_tooth = (uint8*)fs_etpu_malloc(cam_edge_count);
    if(cpba8_cam_edge_tooth == 0)
    {
      return(FS_ETPU_ERROR_MALLOC);
    }
  ...
  eTPU->CHAN[chan_num_crank].SCR.R =
       (uint32)p_tg_instance->polarity_crank + FS_ETPU_TG_FM1_CRANK;
  eTPU->CHAN[chan_num_cam].SCR.R =
       (uint32)p_tg_instance->polarity_cam + FS_ETPU_TG_FM1_CAM;
  ...
  *(cpba + ((FS_ETPU_TG_OFFSET_P_CAM_TOOTH_FIRST   - 1)>>2)) = (uint32)cpba8_cam_edge_tooth - fs_etpu_data_ram_start;
  *(cpba + ((FS_ETPU_TG_OFFSET_P_CAM_TOOTH         - 1)>>2)) = 0;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_TEETH_TILL_GAP     ) = p_tg_instance->teeth_till_gap;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_TEETH_IN_GAP       ) = p_tg_instance->teeth_in_gap;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_TEETH_PER_CYCLE    ) = p_tg_instance->teeth_per_cycle;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_TOOTH_COUNTER_GAP  ) = 0;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_TOOTH_COUNTER_CYCLE) = 0;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_CAM_CHAN           ) = chan_num_cam;
  *((uint8*)cpba + FS_ETPU_TG_OFFSET_GENERATION_DISABLE ) = p_tg_config->generation_disable;
  ...
  struct tg_states_t   *p_tg_states)
{
  ...
  p_tg_states->tooth_counter_cycle =
      *((uint8*)cpba + FS_ETPU_TG_OFFSET_TOOTH_COUNTER_CYCLE);
