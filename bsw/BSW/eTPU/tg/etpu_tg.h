...
{
  const uint8  chan_num_crank; /**< Channel number of the TG Crank channel. */
  const uint8  chan_num_cam;   /**< Channel number of the TG Cam channel. */
  const uint8  priority;  /**< Channel priority for the TG channel. */
  const uint8  polarity_crank;  /**< Initial Crank output polarity.
    It can be one of:
    - @ref FS_ETPU_TG_FM0_POLARITY_LOW
    - @ref FS_ETPU_TG_FM0_POLARITY_HIGH */
  const uint8  polarity_cam;  /**< Initial cam output polarity.
    It can be one of:
    - @ref FS_ETPU_TG_FM0_POLARITY_LOW
    - @ref FS_ETPU_TG_FM0_POLARITY_HIGH */
  ...
  const uint8  cam_edge_count;  /**< The count of Cam edges within an engine
    cycle.*/
  const uint8  *p_cam_edge_tooth; /**< Pointer to the first item of an array
    ...
    space will start from, e.g. 0xC3FC8100. */
        uint8  *cpba8_cam_edge_tooth; /**< Base address of the array of tooth
...
struct tg_config_t
{
  ...
  fract24_t accel_ratio;  /**< A signed fractional value
   ...
   uint8  generation_disable;     /**< This parameter disables/enables
...
struct tg_states_t
{
  uint8  tooth_counter_cycle;  /**< This is the actual value of the tooth
...
*******************************************************************************/
/* Initialize */
uint32 fs_etpu_tg_init(
  struct tg_instance_t *p_tg_instance,
  struct tg_config_t   *p_tg_config);

/* Change configuration */
uint32 fs_etpu_tg_config(
  struct tg_instance_t *p_tg_instance,
  struct tg_config_t   *p_tg_config);

/* Get states */
uint32 fs_etpu_tg_get_states(
  struct tg_instance_t *p_tg_instance,
