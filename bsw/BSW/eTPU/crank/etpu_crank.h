...
{
  const uint8  chan_num;  /**< Channel number of the CRANK channel. */
  const uint8  priority;  /**< Channel priority for the CRANK channel. */
  const uint8  polarity;  /**< CRANK transition polarity.
  ...
  const uint8  teeth_till_gap; /**< A number of physical teeth between 2 gaps.
    This is the number of physical teeth on the crank wheel divided by
    the number of (equally spaced) gaps on the wheel. */
  const uint8  teeth_in_gap; /**< A number of missing teeth in one gap.
  ...
  const uint8  teeth_per_cycle; /**< A number of teeth (including missing
  ...
  const uint8  log_tooth_periods; /**< An option to record tooth_periods.
    It can be one of:
    - @ref FS_ETPU_CRANK_FM1_TOOTH_PERIODS_LOG_OFF
    - @ref FS_ETPU_CRANK_FM1_TOOTH_PERIODS_LOG_ON */
  const uint32 link_cam;  /**< Set of 4 link numbers to send to reset the Cam
    log. Up to 4 Cam channel numbers can be used. In case of a single Cam on
    channel 1, use 0x01010101. */
  const uint32 link_1;    /**< The first  set of 4 link numbers to send when
    stall conditions accure. */
  const uint32 link_2;    /**< The second set of 4 link numbers to send when
    stall conditions accure. */
  const uint32 link_3;    /**< The third  set of 4 link numbers to send when
    stall conditions accure. */
  const uint32 link_4;    /**< The fourth set of 4 link numbers to send when
    stall conditions accure. */
        uint32 *cpba;     /**< Channel parameter base address.
    ...
    space will start from, e.g. 0xC3FC8100. */
        uint32 *cpba_tooth_period_log; /**< Base address of the CRANK tooth
...
{
        uint8  teeth_per_sync; /**< A number of teeth (including the
    ...
    which teeth are ignored. */
        uint8  blank_teeth; /**< A number of teeth ignored after
    initialization. */
...
{
        uint8 error;      /**< This is the error status of CRANK. It includes
    ...
    The eTPU sets the error flags, the CPU clears them after reading. */
        uint8 state; /**< This is the state of CRANK. It can be one of:
    ...
    - @ref FS_ETPU_CRANK_TOOTH_AFTER_GAP  */
        uint8 eng_pos_state; /**< This is the global engine position state.
    It can be one of:
    ...
    - @ref FS_ETPU_ENG_POS_FULL_SYNC  */
        uint8 tooth_counter_gap; /**< The actual number of the tooth counter
    which counts from 1 to teeth_till_gap and resets on every gap. */
        uint8 tooth_counter_cycle; /**< The actual number of the tooth counter
