  ...
  struct crank_config_t   *p_crank_config)
{
  ...
  eTPU->CHAN[chan_num].CR.R =
       ...
       (((uint32)cpba - fs_etpu_data_ram_start) >> 3);
  eTPU->CHAN[chan_num].SCR.R = (uint32)p_crank_instance->polarity
	                             + (uint32)p_crank_instance->log_tooth_periods;
  ...
  *(cpba + ((FS_ETPU_CRANK_OFFSET_TOOTH_PERIOD_LOG        - 1)>>2)) = (uint32)cpba_log - fs_etpu_data_ram_start;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TEETH_TILL_GAP     ) = p_crank_instance->teeth_till_gap;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TEETH_IN_GAP       ) = p_crank_instance->teeth_in_gap;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TEETH_PER_CYCLE    ) = p_crank_instance->teeth_per_cycle;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TEETH_PER_SYNC     ) = p_crank_config->teeth_per_sync;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TOOTH_COUNTER_GAP  ) = 0;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TOOTH_COUNTER_CYCLE) = 0;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_BLANK_TEETH        ) = p_crank_config->blank_teeth;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_STATE              ) = FS_ETPU_CRANK_SEEK;
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_ERROR              ) = FS_ETPU_CRANK_ERR_NO_ERROR;
  /* 16-bit */
  misscnt_mask = p_crank_instance->teeth_in_gap << 13;
  misscnt_mask = (misscnt_mask & 0x6000) | ((misscnt_mask & 0x8000)>>5);
  *((uint16*)cpba + (FS_ETPU_CRANK_OFFSET_MISSCNT_MASK>>1)) = misscnt_mask;
  /* 32-bit */
  ...
  *((uint32*)fs_etpu_data_ram_start + ((FS_ETPU_OFFSET_ENG_CYCLE_TCR2_TICKS -1)>>2)) =
    p_crank_instance->tcr2_ticks_per_tooth * p_crank_instance->teeth_per_cycle;
  *((uint32*)fs_etpu_data_ram_start + ((FS_ETPU_OFFSET_ENG_CYCLE_TCR2_START -1)>>2)) = 0;
  *((uint8*)fs_etpu_data_ram_start + FS_ETPU_OFFSET_ENG_POS_STATE) = FS_ETPU_ENG_POS_SEEK;
  eTPU->TBCR_A.B.AM = 2;
  /* Write HSR */
...
*
*******************************************************************************/
uint32 fs_etpu_crank_get_states(
  struct crank_instance_t *p_crank_instance,
  struct crank_states_t   *p_crank_states)
{
  ...
  p_crank_states->state               = *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_STATE);
  p_crank_states->eng_pos_state       = *((uint8*)fs_etpu_data_ram_start + FS_ETPU_OFFSET_ENG_POS_STATE);
  p_crank_states->tooth_counter_gap   = *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TOOTH_COUNTER_GAP);
  p_crank_states->tooth_counter_cycle = *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_TOOTH_COUNTER_CYCLE);
  p_crank_states->last_tooth_period   = *(cpbae + ((FS_ETPU_CRANK_OFFSET_LAST_TOOTH_PERIOD - 1)>>2));
  p_crank_states->error              |= *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_ERROR);
  /* Clear Crank error */
  *((uint8*)cpba + FS_ETPU_CRANK_OFFSET_ERROR) = 0;
