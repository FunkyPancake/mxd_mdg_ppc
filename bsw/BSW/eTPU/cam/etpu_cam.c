  ...
  struct cam_config_t   *p_cam_config)
{
  ...
  eTPU->CHAN[chan_num].SCR.R = (uint32)p_cam_config->mode;
  ...
  *(cpba + ((FS_ETPU_CAM_OFFSET_LOG       - 1)>>2)) = (uint32)cpba_log - fs_etpu_data_ram_start;
  *((uint8*)cpba + FS_ETPU_CAM_OFFSET_ERROR     ) = FS_ETPU_CAM_ERROR_NO;
...
{
  /* Write FM bits */
  eTPU->CHAN[p_cam_instance->chan_num].SCR.R = (uint32)p_cam_config->mode;
  ...
  struct cam_states_t   *p_cam_states)
{
  ...
  p_cam_states->error     = *((uint8*)cpba + FS_ETPU_CAM_OFFSET_ERROR);

  /* Clear CAM error flags */
  *((uint8*)cpba + FS_ETPU_CAM_OFFSET_ERROR) = 0;
