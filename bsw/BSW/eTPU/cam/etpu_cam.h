...
{
  const uint8  chan_num;  /**< Channel number of the CAM channel. */
  const uint8  priority;  /**< Channel priority for the CAM channel. */
  const uint8  log_size;  /**< CAM log array size as a number of logged
                                 items. */
    ...
    space will start from, e.g. 0xC3FC8100. */
        uint32 *cpba_log; /**< Pointer to the CAM log buffer in eTPU DATA RAM.
...
struct cam_states_t
{
        uint8 error;      /**< This is the error status of CAM. It includes
    ...
    The eTPU sets the error flags, the CPU clears them after reading. */
        uint8 log_count; /**< This is a count of transitions logged during
    the last completed engine cycle. */
        uint8 log_idx; /**< This is an index of the first free position
...
*******************************************************************************/
/* Initialize */
uint32 fs_etpu_cam_init(
  struct cam_instance_t *p_cam_instance,
  struct cam_config_t   *p_cam_config);

/* Change configuration */
uint32 fs_etpu_cam_config(
  struct cam_instance_t *p_cam_instance,
  struct cam_config_t   *p_cam_config);

/* Get states */
uint32 fs_etpu_cam_get_states(
  struct cam_instance_t *p_cam_instance,
  struct cam_states_t   *p_cam_states);

/* Copy log */
uint32 *fs_etpu_cam_copy_log(
  struct cam_instance_t *p_cam_instance,
               uint32 *p_cam_log);

/* Reset log */
uint32 fs_etpu_cam_reset_log(
  struct cam_instance_t *p_cam_instance);
