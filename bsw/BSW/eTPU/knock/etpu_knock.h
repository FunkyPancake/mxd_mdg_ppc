...
{
  const uint8  chan_num;  /**< Channel number of the KNOCK channel. */
  const uint8  priority;  /**< Channel priority for the KNOCK channel. */
  const uint8  polarity;  /**< KNOCK polarity. It can one any of:
  ...
  const uint24_t  tdc_angle; /**< The cylinder Top Dead Center as a number
    ...
    space will start from, e.g. 0xC3FC8100. */
        uint32 *cpba_windows; /**< Base address of the knock windows array in
...
{
  uint8  mode;  /**< KNOCK mode. It can one of:
    - @ref FS_ETPU_KNOCK_FM1_MODE_GATE
    - @ref FS_ETPU_KNOCK_FM1_MODE_TRIGGER */
  uint8  window_count;  /**< The count of knock windows. */
  ...
  uint8  irq_dma_options;  /**< Enables to generate channel interrupt and/or
