  ...
  struct knock_config_t     *p_knock_config)
{
  ...
  eTPU->CHAN[chan_num].SCR.R = (uint32)p_knock_instance->polarity
                                       + p_knock_config->mode;

  /* Write channel parameters */
  /* 24-bit */
  *(cpba + ((FS_ETPU_KNOCK_OFFSET_P_WINDOW_FIRST    - 1)>>2)) = (uint32)cpba_windows - fs_etpu_data_ram_start;
  ...
  *(cpba + ((FS_ETPU_KNOCK_OFFSET_TRIGGER_PERIOD    - 1)>>2)) = p_knock_config->trigger_period;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_WINDOW_COUNT      ) = window_count;
  *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_WINDOW_COUNTER    ) = 0;
  *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_IRQ_DMA_OPTIONS   ) = p_knock_config->irq_dma_options;
  ...
  struct knock_config_t   *p_knock_config)
{
  ...
  *(cpbae + ((FS_ETPU_KNOCK_OFFSET_TRIGGER_PERIOD - 1)>>2)) = p_knock_config->trigger_period;
  /* 8-bit */
  *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_IRQ_DMA_OPTIONS)  = p_knock_config->irq_dma_options;
  *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_WINDOW_COUNT)     = p_knock_config->window_count;
...
uint32 fs_etpu_knock_get_states(
  struct knock_instance_t *p_knock_instance,
   uint8 *window_counter)
{
  uint32 *cpba;
  uint32 *cpbae;
  ...
  *window_counter = *((uint8*)cpba + FS_ETPU_KNOCK_OFFSET_WINDOW_COUNTER);
  /* Clear FUEL error */
  return(0);
}
/*******************************************************************************
